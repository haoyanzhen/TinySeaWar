...
- `docs/15_battle_level_design.md`：自定义战斗、23 个教学/挑战关、编成、地图预设、声明式目标、接替增援、进度存档、解锁和验收设计。改战斗模式、关卡结构或进度设计时先读。
...
- `docs/19_battle_simulator_design.md`：无图形战斗模拟、实验清单、固定种子、统计口径、报告和调参验收。改批量模拟或模拟报告时先读。
...
- `docs/36_balance_testing_design.md`：平衡性测试的实验输入、输出、参数、统计、结论边界和实施状态。改平衡实验、样本、比较口径或平衡报告时先读。
...
- `docs/technical/t02_level_objective_reinforcement_progress_solution.md`：23 个教学/挑战关的声明式目标、接替增援、结算与进度存档技术方案。改关卡目标运行时、增援编排或闯关进度时先读；当前是已定稿未实施方案。
...
- **玩法策划未完成**：已有核心规则、关卡和 AI 设计基础，23 个教学/挑战关的目标、接替增援和进度存档契约已经定稿但尚未实现；自定义编成、正式闯关数据与菜单、奖励表现、舰种生态和动态平衡仍未形成运行闭环。
