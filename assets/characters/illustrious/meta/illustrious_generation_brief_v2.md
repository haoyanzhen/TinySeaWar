# illustrious Codex native-alpha production brief — 2026-09-20
...
Character origin: 光辉号 HMS Illustrious.
Faction: 英系. MVP ship class: 航母. Level: 3 级.
Personality: 威严旗舰（沉稳守护）. Combat role: 装甲航母与耐久航空.
Core visual direction: 低饱和皇家蓝重礼装、厚肩甲和稳定直立姿态；装甲飞行甲板短而厚，机库与防空阵列外露；起飞灯节奏平稳，受击反馈以甲板火花而非大幅摇摆表现.
Required MVP asset focus: 全身/半身立绘、高规格技能 cut-in、装甲甲板/机库、舰载机小图、双起飞点与回收点、防空中心、装甲甲板受击层、稳定出击和耐久航空动画.
Expected object inventory from the authoritative postprocess plan: {"aircraft_launch": {"instances": 2}, "aircraft_recovery": {"instances": 1}}.
...
Use the accepted style anchor as the only identity reference. Create exactly eight separated cells in a 4x2 grid: portrait, small portrait, chibi head, default expression, serious expression, hit expression, skill icon for `armored_flight_deck`, and abstract `carrier` class icon. True transparent background, no checkerboard, no matte, no text, no overlapping cells.

## Battle 4x2 sheet

Use the accepted style anchor as the only identity reference. Create exactly eight isolated orthographic game assets in a 4x2 grid, in this order: battle_body_r, battle_rig_base, battle_flight_deck, battle_hangar, battle_aircraft_group_01, battle_aircraft_launch_marker, battle_aircraft_recovery_marker, battle_aa_center. True transparent background, no checkerboard or matte. Keep every component separated with clear empty transparent margins and readable pivots.
