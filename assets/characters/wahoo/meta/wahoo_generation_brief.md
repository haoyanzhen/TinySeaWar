# 刺尾鱼号 USS Wahoo SS-238 Phase 2 Generation Brief
...
Character origin: 刺尾鱼号 USS Wahoo SS-238.
Faction: 美系. MVP ship class: 潜艇. Level: 2 级.
Personality: 神秘潜行（锐利猎手）. Combat role: 高风险连续伏击.
Core visual direction: 黑蓝潜航服与鲨鳍式短披肩；长艇身底座、六艏四艉鱼雷口和巡逻刻痕形成攻击性轮廓；氧气警示灯由蓝转橙表达高风险循环.
Required MVP asset focus: 全身/半身立绘、技能 cut-in、长艇身低矮舰装、艏/艉鱼雷口、声呐与氧气指示、鲨形水下阴影、连续装填脉冲和前后雷击动画.
...
Use the accepted style anchor as the only identity reference. Create exactly eight separated cells in a 4x2 grid: portrait, small portrait, chibi head, default expression, serious expression, hit expression, skill icon for `continuous_hunt`, and abstract `submarine` class icon. Flat reserved green background, no text, no overlapping cells.
...
Create exactly eight isolated character-specific VFX overlays in a 2x4 grid, in this order: shark_shadow, reload_pulse, sonar_pulse, torpedo_launch_flash, torpedo_trail, underwater_shadow, dive_ripple, skill_aura. Reusable projectile bodies and impacts are not duplicated. Flat reserved green background, no text, crisp small-scale game VFX shapes.
