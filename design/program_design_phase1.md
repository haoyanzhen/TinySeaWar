...
- `design/domain_design_phase1.md`
- `docs/technique.md`
...
`ShipUnitView` 只维护 `entity_id`、资产装配和表现状态，通过 `BattleSnapshot` 同步位置、航向、生命和可见性。移动、生命、侦查、武器、技能、状态和 AI 的真状态属于 Domain，详细规则见 `design/domain_design_phase1.md`。
...
事件用于表现、UI 和统计记录。高频位置、航向、生命和装填显示通过只读 `BattleSnapshot` 更新，不经过全局事件总线。事件的完整字段、顺序和状态所有权见 `design/domain_design_phase1.md`。
...
1. 选择一个或多个我方单位，点击战场下达移动目标。
2. 选择已发现敌人，下达集火指令。
3. 点击角色技能按钮，完成自身、单位或区域目标选择。
