...
该对象既是量化公式真源，也是当前首轮运行时实现的评分内核。`BattleSession` 已接入量化目标、舰种模式迟滞、发现后战术、主要武器窗口、技能收益、即时生存和港湾兼容策略；玩家受限辅助控制与 `Z/X/C/V/G` 操作也已接入。尚未完成的部分包括正式 `AIObservation` 对象、弹性编组/阵列、伤害与效果预留、多候选掩体、难度 Profile 和完整动态指标，因而整体状态仍是“部分完成”。
...
| 即时生存 | 是否必须立刻保命 | 规避鱼雷、脱离岸线、让路、边界逃逸 |
...
| 即时生存 | 每 Tick | 鱼雷进入反应窗、未来岸线碰撞、硬边界风险 |
| 武器/技能/移动合法性 | 每 Tick | 始终由战斗系统权威校验 |

玩家受限辅助 AI 不运行舰队方案、任务、编组和战略模式周期；局部执行和被发现动作可沿用 `0.5s`，即时生存仍每 Tick 检查。
...
- 航线由目标位置转换为短航路；除紧急规避外，不每 Tick 重发目标点。
- 目标、模式和航路都使用驻留和滞回，避免射程边缘反复改变意图。
...
BoundaryEscape
HazardExit
EmergencySurface（潜航闭环完成后）
```

同一 Tick 多个候选同时触发时选择最高风险；相差小于 `5` 分时按上述顺序处理。默认进入阈值 `65`，退出阈值 `35`，危险解除后保持 `0.8s` 恢复时间。
...
规避不只选择“远离鱼雷”：生成左转、右转、加速穿越、减速让过和向掩体侧脱离等候选，分别计算未来最近距离、岸线风险、友军冲突和任务偏离。鱼雷危险优先，但不能把舰船主动送上岸。

#### 岸线风险

```text
shore_risk = 100 * (
  0.35 * (1 - clearance_ratio)
  + 0.30 * future_collision
  + 0.20 * (1 - turn_room_ratio)
  + 0.15 * current_toward_shore
)
```

- `future_collision` 使用当前速度、海流和 `1.5-2.5s` 预测位置。
- 风险 `>= 65` 时禁止继续缩短岸线距离；`>= 75` 时强制沿岸切向或向外脱离。
- Domain 岸线扫掠仍是最后安全网，AI 不依赖碰撞后滑动完成正常航行。

#### 边界和友军

- 边界风险沿用当前动态转弯半径设计：`>= 0.8` 进入强制逃逸。
...
- 普通路线每 `0.8-1.5s` 或目标偏离阈值后重算。
- 即时生存可以立刻生成短规避航点，但危险解除后重新规划主路线。
- 数秒内进度不足时依次尝试：减速让路、选择外侧航点、重算完整短航路、进入强制脱离。
...
当前首轮实现由 `BattleSession` 协调并调用 `ai_quantitative_model.gd`；上列独立控制器、观察对象、编组协调器和攻击规划器仍是后续从会话类中拆分的目标位置，不能据路径不存在误判量化 AI 尚未运行。
...
截至 2026-06-29，执行结果为 `50` 项检查、`11` 组场景全部通过。

| 场景组 | 关键数值 | 验证结论 |
|---|---|---|
| 即时生存 | 致命鱼雷 `82.17`；安全掠过 `19.50`；岸线风险 `89.80` | 只对已发现且真实危险的鱼雷打断；未来触岸会强制脱离 |
...
| 近岸与路线 | 可用掩体 `86.95`；死胡同 `17.40`；暴露直路 `63.40`；安全绕路 `84.15` | 不把无出口岸湾当掩体，安全绕路能胜过最短路线 |
| 层级优先 | 鱼雷打断 `86`；设施防守 `82`；旗舰护航 `72` | 即时生存、任务、编组、模式按既定层级生效 |
| 滞回序列 | 驻留拒绝、第一次确认、第二次切换、紧急立即切换均通过 | 避免瞬时分数波动导致模式横跳 |
...
以上纯计算测试验证“量化模型对给定态势是否选择预期行为”。首轮控制状态、路径、主武器自动窗口、技能禁用和即时边界规避已经加入核心运行时回归；完整 AI 仍需要两级验收：

1. **控制器集成测试**：用构造的 `AIObservation` 验证命令、记忆、驻留、目标预留和打断恢复。
2. **整局动态测试**：在 1v1、3v3、5v5、11v11 和港湾关卡用固定种子运行多局模拟。

玩家辅助控制当前已覆盖默认开关、`Z` 多航点、`X/C/V` 状态、舰队范围命令、`G` 镜头入口、自动主武器窗口、技能恒不自动和即时边界规避。后续仍需补齐真实鱼雷打断后的路径恢复、混合开关组合、输入级 `Cmd/Alt` 自动化，以及改变编组、任务、设施或天气收益输入时局部决策不变的集成断言。
...
首轮建议每个代表场景至少 `30` 个固定种子；权重修改后同一组种子回归。行为验收优先于胜率：即使胜率接近 50%，若 AI 高频撞岸、乱放技能或全队争抢同一设施，也不能通过。
...
1. **部分完成**：投射物已按阵营过滤，正式 `AIObservation` 对象待拆分。
2. **首轮完成**：已建立运行时记忆、决策周期、模式迟滞和关键解释事件；目标迟滞仍待补齐。
...
2. **部分完成**：技能预期收益和开火纪律已接入，显式连招与效果预留待完成。
3. **首轮完成**：已接入鱼雷 CPA、岸线风险和边界打断，仍需多种子动态验收。
4. **未完成**：多候选路线、掩体评分和卡住恢复仍待接入。
5. **已完成**：玩家 `Z` 路线、`X/C/V` 开关、`Cmd/Alt` 舰队范围和 `G` 镜头跟踪已接入。

### 阶段 C：任务与编组

1. 把当前设施兼容策略升级为任务分配、饱和限制和中断恢复。
2. 建立固定编组职责和弹性阵位。
3. 建立目标伤害预留、技能效果预留和协同窗口。
4. 建立设施内守、入口拦截和远程火援分工。

### 阶段 D：动态验收

1. 为开阔海域和港湾建立固定种子场景集。
2. 将 AI 行为指标接入批量模拟报告。
3. 按舰种、角色、规模、天气、设施和地形分层验收。
