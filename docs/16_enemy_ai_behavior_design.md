...
本文是 Tiny Sea War 敌方 AI 的统一设计入口，定义：
...
该对象是设计的可执行规格，尚不代表全部行为已经接入 `BattleSession`。当前运行时只有基础目标选择、接近/后退、自动技能，以及港湾设施、维修、机场、背风和已知雷区的兼容策略。
...
- 投射物必须增加阵营可见过滤；现有全量投射物快照不能直接作为正式 AI 输入。
- 鱼雷预警技能只改变发现时机或置信度，不改变鱼雷真实运动。
...
当前已建立 `ai_quantitative_model.gd`；其余是后续运行时接入位置。
...
2026-06-28 首轮执行结果：`35` 项检查、`10` 组场景全部通过。
...
以上测试验证“量化模型对给定态势是否选择预期行为”，不等于完整运行时 AI 已接入。正式实现还需要两级验收：
...
1. 建立阵营过滤的 `AIObservation`，补投射物可见性。
2. 建立 `AIRuntimeState`、决策周期、模式/目标滞回和解释事件。
3. 接入已验证的量化模型，替换当前统一接近—后退逻辑。
4. 让敌方提交合法 `ManualPrimary` 命令。

### 阶段 B：单舰完整响应

1. 接入 `Attack/Defend/Kite`。
2. 接入技能预期收益、开火纪律和显式连招。
3. 接入鱼雷 CPA、岸线风险和边界打断。
4. 接入多候选路线、掩体评分和卡住恢复。
