...
当前实施与正式复验只以普通潜艇六阶段分支为关闭范围。特殊潜射艇跳过 `SurfaceForAttack` 的分支保留既有规则和基础回归，但其战术质量、整局循环与正式样本暂不考虑；潜艇友军雷道硬淘汰同样暂不考虑，当前只保留 `observed_friendly_risk` 诊断事实，不作为窗口降分、候选拒绝或本轮验收失败条件。恢复任一范围前必须另行更新本节验收口径并获得对应大样本授权。
...
1. `Search -> Approach -> SurfaceForAttack -> AttackRun -> BreakContact -> RecoverOxygen -> Search` 的普通潜艇六阶段全循环；重新下潜只作为 `RecoverOxygen` 内的深度请求。特殊潜射艇跳过 `SurfaceForAttack` 的整局分支暂不纳入本轮正式验收，只保留既有基础规则回归。
2. 前管合法、后管合法、两者均合法、一座装填、一座禁用和权威数组预先乱序；评分、预留、事件和实际发射必须保持同一 `weapon_state_instance_id`。
3. 无可见目标、只有合法残影、目标在射程外、射界外、深度不合法和出口失效时均不得提交鱼雷；首轮潜艇窗口的 `friendly_risk` 必须固定为 `0`，同时保留大于 `0` 的 `observed_friendly_risk` 诊断证据，证明友军雷道既未降分也未成为硬门禁。友军当前位置/预测位置与雷道相交的候选硬淘汰暂不考虑，不属于本轮正式验收条件。
...
- 潜艇能在合法情报与公共规则下完成搜索、接近、普通上浮、同源发射器雷击、脱离、补氧和重新下潜；无合法解时能说明卡在哪一层，而不是长期零开火。特殊潜射与友军雷道硬淘汰按 5.6.1 的暂缓范围单独管理，不阻塞本轮普通潜艇验收。
