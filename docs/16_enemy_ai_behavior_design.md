...
| 鱼雷威胁与即时生存 | 已知投射物、CPA、伤害潜力、转向时间、岸线与边界 | 已实现 | 鱼雷/岸线量化场景、隐藏发射舰隔离检查 |
...
公式结构已不再缺项；剩余工作属于参数标定和动态证据覆盖，而不是继续添加未接线的评分项。港湾已按 `1 -> 3 -> 20` 阶梯完成行为验收；当前未通过项是舰种、天气、设施和地形的分层平衡结论，性能优化另列为非阻断工作。
...
4. **首轮完成**：难度 Profile 提供 `3/5/7` 路线候选预算；近岸模式会评估带合法出口的掩体，连续无进展会触发 `PathRecovery`。12 项专项检查通过，港湾批量性能仍待优化。
...
1. **部分完成**：开阔海域已有历史 40 局侧别交换集；港湾专项已完成 20 局正式样本，后续新增动态验收统一按 `1 -> 3 -> 20` 阶梯执行。
