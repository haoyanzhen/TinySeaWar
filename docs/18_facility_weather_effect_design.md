...
| 岸防炮 | `ui_marker_facility_coastal_battery.png` | `facility.coastal_battery.*` | 使用正式舰炮、射界、装填、命中和伤害规则；需要合法阵营接触，压制时停火并暂停装填 |
...
| 飑线 | `ui_marker_environment_rain_squall.png` | `rain_squall_mask/edge` | 局部光学能见度乘以 `0.72`，航空条件至少为 `Restricted` |
| 高海况 | `ui_marker_environment_high_sea.png` | `high_sea_foam_mask` | 将局部海况向 4 级插值，最终海况影响航速、命中和航空 |
| 背风水域 | `ui_marker_environment_lee_water.png` | `lee_water_mask` | 最终海况降低 2 级，为恶劣天气下的近岸稳定航路 |
...
| 天气 | 基础海况 | 光学倍率 | 额外命中 | 航空延迟 | 航空条件 | 战术定位 |
| --- | ---: | ---: | ---: | ---: | --- | --- |
| 晴朗 | 1 | `1.00` | `0.00` | `1.00` | Normal | 信息充分、机动稳定，作为标准基线 |
| 多云 | 2 | `0.96` | `0.00` | `1.04` | Normal | 轻微压低远距离发现，基本不改变武器节奏 |
| 阴云 | 3 | `0.88` | `-0.02` | `1.10` | Normal | 视距下降并进入 3 级海况，鼓励接近和稳健航路 |
| 雨 | 3 | `0.76` | `-0.05` | `1.25` | Restricted | 接触距离缩短、航空变慢，局部突击价值提高 |
| 雷雨 | 5 | `0.58` | `-0.10` | `1.60` | Severe | 高海况、低能见度和航空受限并存，背风与设施争夺更关键 |
...
因此天气本身的观测/操作困难和海况造成的平台困难可以同时存在。数值采用确定相加或相乘，不额外抽取随机数。
