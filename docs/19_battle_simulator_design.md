...
| 用途 | 每代表场景种子数 | 说明 |
| --- | ---: | --- |
| 冒烟 | 3-6 | 只检查能启动、能结束、无断言 |
| 固定回归 | 10-30 | 使用稳定种子，关注未解释漂移 |
| AI 动态验收 | 至少 30 | 与 `docs/16_enemy_ai_behavior_design.md` 一致 |
| 初步平衡筛查 | 50-100 | 用于发现大幅差异，不定最终结论 |
| 关键 A/B 决策 | 200 以上 | 结合区间和分层结果判断 |

表中是起点，不是充分性的保证。结果接近门槛、方差过高或稀有事件重要时应增加样本。
...
- 11v11 与港湾关卡允许较慢，但批量 30 个种子不应依赖人工等待或场景渲染。
...
| `sim.ai.dynamic_representative` | 五个当前关卡 | 每关至少 30 | AI 行为动态验收 |
