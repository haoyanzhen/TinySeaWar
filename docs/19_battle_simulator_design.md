...
- 长时间消极行为的累计持续时间、接敌压力触发次数、触发后平均接敌时间和连续 `20s` 无显著位移次数；单局保留触发分量、豁免原因与评分修正事件，聚合报告输出四项摘要。
...
平衡性测试的输入、输出、参数分层、测试族、结论边界和实施状态以 `docs/36_balance_testing_design.md` 为唯一设计真源。本文件只保留模拟器通用的随机性、聚合、样本量和报告实现说明。
