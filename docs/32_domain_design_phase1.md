...
`FirePrimaryWeapon` 可以携带可选 `weapon_state_instance_id` 作为首选发射器提示。省略时保持既有玩家命令语义；提供时 Domain 必须按该实例重新验证启用、装填、管组间隔、目标、深度、射程、射界和路径合法性，提示失效时拒绝，不能静默改用另一座武器。`WeaponFired` 事实记录实际 `weapon_state_instance_id`。
...
- **ObservationService**：侦查、隐蔽、深度派生距离、视线、暴露与阵营接触。
- **WeaponService**：装填、目标/射界/发射深度合法性、攻击展开和弹药切换。
...
1. 读取并稳定排序本 Tick 命令。
2. 校验命令，更新控制意图、路线、目标、潜艇深度转换、技能或设施任务。
3. 推进全局环境、区域上下文和设施生命周期。
4. 推进深度转换、最短保持与氧气资源，零氧时强制上浮。
5. 更新观察与合法目标信息，供 AI 下一决策窗和本 Tick 行为使用。
6. 计算导航/控制输入并推进单位运动，执行地形与边界权威校验。
7. 推进装填、冷却、技能效果和持续状态。
8. 展开武器与技能攻击，生成落点事实或实体投射物。
9. 推进投射物并收集接触候选。
10. 统一结算命中、装甲、伤害、沉没与统计。
11. 结算关卡取消条件、旗舰胜负和时限结果。
12. 生成观察者快照与有序事件，递增 Tick。
