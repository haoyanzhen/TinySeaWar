...
本文设计已在 `level.prototype_harbor_3v3` 形成包含设施与局部环境的完整运行闭环：硬地形查询、共享导航、浅水/航道、动态环境战术结果、观察与岸炮、补给维修、机场支援、水雷、设施生命周期、AI 环境意图、表现、小地图、调试与 QA 均已接入。其余九套岛屿也已各自生成运行时地图、共享导航、审核出生、小地图遮罩和 3v3 验证入口；它们当前覆盖通用地形主链路，不包含港湾专属设施与局部环境，正式关卡节奏仍以 `docs/15_battle_level_design.md` 后续编排和 `docs/00_project_status.md` 验收状态为准。
...
雷达站继续遵守 13.8 节约束：在独立雷达传感器规则建立前保持 `enabled = false`，不会用光学观察规则冒充雷达。十套岛屿均有审核模板、运行时地图、出生、导航、小地图和 3v3 验证入口；只有港湾入口配置环境区、设施与水雷，其余九套按无设施近岸战处理，且尚未完成正式教学/挑战关的节奏和平衡验收。
...
- 港湾默认敌方、激活，不声明 `AreaControl`，水面舰不能直接占领；`destroyable=false`，HP 下限、累计压制阈值 `140`、恢复 `15s` 与静默/失能覆盖层由通用设施生命周期表达。
- 玩家或 AI 提交同结构的支援请求。
- Domain 校验设施状态、使用次数、冷却、目标区域和环境条件。
- 成功后创建具有到达过程的支援任务或航空实体。
- 请求后先进入 `Preparing`，到达 `launch_time` 且机场仍可运行时进入 `EnRoute`；准备阶段遇到机场压制、失能或静默立即取消，已经离场的任务不再依赖机场，继续至到达结算。
- 同阵营任一已声明通信站可用时机场运行；通信暂时压制使机场 `Disabled`，恢复后重算，全部通信被摧毁或转为异阵营后进入 `Silent` 并拒绝新任务。
...
- 港湾初态为中立、休眠，可占领、压制和摧毁；激活后只接受同阵营舰船，单泊位由 `service_state` 独占。
- 申请时同时校验舰船位于审核后的泊位水域、速度不超过 `10`、航向与泊位方向偏差不超过 `25°`。通过后记录进坞位置并进入 `Docked`；下一次服务推进切换到 `Servicing` 后，九秒维修计时才开始。
- `Docked/Servicing` 期间舰船定点保持，不读取推进、水流或普通舰体分离位移；其他舰船仍以其为碰撞障碍。定点保持不提供攻击、伤害、沉没或胜负免疫。
- 有效移动指令和显式离泊先取消服务再恢复普通运动；驶出泊位、单次最终伤害达到最大 HP 的 `12%`、设施受击/压制/摧毁、依赖失效或维修舰沉没均中断并清零进度。沉没优先报告 `UNIT_SUNK`，不会被重击原因覆盖。
- 服务完成后恢复最大 HP 的 `28%`，但结果不超过最大 HP 的 `80%`；随后释放泊位并回到定点待命。维修只修改服务舰 HP，不改变维修泊位以外的任何设施。
