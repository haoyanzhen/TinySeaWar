...
> 状态更新（2026-06-27）：运行时注册表现有 48 名角色定义，现有四个可玩关卡仍覆盖第一期 24 名；下文关于“仅 6 名角色已配置”、两个关卡和 `enterprise_cv6` 未进入运行数据的内容仅保留为 2026-06-19 的历史快照，不再代表当前状态。最新完成度见 `docs/00_project_status.md`，代码入口见 `docs/34_implementation_map.md`。
