...
> 状态更新（2026-06-23）：运行时舰船数据已扩展为完整 24 人，且 11v11 使用 22 名不同角色；下文关于“仅 6 名角色已配置”和 `enterprise_cv6` 未进入运行数据的内容仅保留为 2026-06-19 的历史快照，不再代表当前状态。最新入口见 `data/ships/expanded_roster_ships.json` 与 `docs/33_implementation_map.md`。
