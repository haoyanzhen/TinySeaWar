...
教学使用 `tutorial_stage_plan` 自然组织遭遇：由航点到达、首次接触、玩家完成前置操作或进入合法武器窗口推进阶段，并通过出生距离、航线、侦查状态和敌方交战意图让战斗逐步展开。阶段可以限制玩家单位选择、自动航行、主/副武器自动开火、弹药、主要武器和技能，也可限制敌方主动攻击策略；每项限制必须在 HUD 明示，并在讲解后开放。不得以静止靶、无敌、锁血或修改伤害/命中规则代替教学阶段控制。
...
挑战首通处理为幂等事务：在内存副本同时合并挑战成功状态和舰船解锁，校验后写临时文件并原子替换正式文件。教学完成只合并对应舰船解锁，不写教学完成记录。重复结算、崩溃恢复和重玩不重复解锁。未知 ID 保留供前向兼容，但不产生当前解锁。
...
- 存档测试覆盖挑战首通、教学舰船解锁、重玩、重复事件、临时文件恢复、版本迁移、未知 ID 和不可信战斗来源。
