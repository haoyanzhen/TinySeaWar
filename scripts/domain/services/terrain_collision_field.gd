...
func configure(metadata: Dictionary, occupancy_bytes: PackedByteArray, distance_bytes: PackedByteArray) -> bool:
...
func query_trajectory_samples(samples: Array, required_clearance: float) -> Dictionary:
		...
		if absi(cell_delta.x) <= 1 and absi(cell_delta.y) <= 1:
			...
			if cell_delta.x != 0 and cell_delta.y != 0:
				...
				if fraction_x <= fraction_y + EPSILON:
					minimum_clearance = minf(minimum_clearance, float(_distance_at_cell(Vector2i(end_cell.x, start_cell.y))))
					cells_visited += 1
				if fraction_y <= fraction_x + EPSILON:
					minimum_clearance = minf(minimum_clearance, float(_distance_at_cell(Vector2i(start_cell.x, end_cell.y))))
					cells_visited += 1
	...
	return {"definitely_clear":definitely_clear, "minimum_by_segment":minimum_by_segment, "cells_by_segment":cells_by_segment}
