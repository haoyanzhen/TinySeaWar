...
func _run() -> void:
	...
	var depth_visual: Dictionary = presentation_settings.get("submarine_depth_visual", {})
	_check(depth_visual.get("transition_easing", "") == "SmoothStep", "submarine depth presentation exposes smooth snapshot-driven transitions")
	_check(depth_visual.get("surface", {}).get("body_tint", []) == [1.0, 1.0, 1.0, 1.0], "surfaced submarines preserve authored body color and opacity")
	_check(depth_visual.get("submerged", {}).get("body_tint", []) == [0.72, 0.85, 0.91, 0.64] and depth_visual.get("submerged", {}).get("rig_tint", []) == [0.56, 0.72, 0.78, 0.52], "submerged body and rig use distinct cool tint and opacity profiles")
