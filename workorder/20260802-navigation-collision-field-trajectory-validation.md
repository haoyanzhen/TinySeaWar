...
restriction_offset / restriction_length
mip_offsets[]
source_checksum
payload_checksum
```

`restriction` 为与基础格一一对应的静态水域限制位，当前位定义为浅水与礁区；它只证明某段遍历格未触及相应限制类型，不保存吃水语义，也不取代权威区域边界。
...
2. 选取 `block_mask` 包含 `ShipMovement` 的障碍多边形，并收集影响舰艇吃水合法性的静态浅水/礁区多边形。
3. 将地图外区域纳入边界距离。
4. 以保守覆盖规则生成基础占用位图。
5. 以保守覆盖规则为相交格写入浅水/礁区限制位；吃水标签仍由运行时解释。
6. 生成距离下界场与可选 Mipmap。
7. 写入稳定排序、固定端序和无时间戳的二进制文件。
8. 写入包含源 checksum、生成器版本和输出 checksum 的 manifest；源 checksum 必须覆盖相关静态限制区。
9. 重复烘焙两次并做字节一致性比较。
10. 用随机点、切线、凹角和长线段与多边形权威查询做差分验证，同时验证限制位不会错误宣称区域确定不相交。
...
TrajectoryTerrainResult = {
  ...
  field_cells_visited,
  traversed_restriction_mask,
  exact_segment_checks
}
```

Mask/SDF/限制位无需保存障碍或区域 ID；发生近岸不确定时，由多边形窄相返回稳定障碍 ID 和法线，静态水域限制不确定时由权威区域查询解释吃水标签。
...
3. 读取每个单元的 `distance_lower_bound`，并 OR 合并静态水域 `restriction_mask`。
4. 若所有单元均满足 `distance_lower_bound > required_clearance`，岸线部分确定安全；否则对该短段调用 `is_movement_segment_clear()` / `first_segment_hit(..., sweep_radius)`。
5. 若合并限制位与当前舰艇吃水阻挡位不相交，静态浅水/礁区部分确定安全；否则调用权威区域查询做精确判断。
6. 超出字段、revision 不一致或数据无效时，岸线与静态限制水域均走各自精确回退。
7. 精确查询无碰撞则继续下一短段；有碰撞则立即返回首个碰撞 Tick 与分数。
...
### 19.2 2026-08-05 局部航迹开销复验

- 保留 `0.1s × 6.0s`、候选数量、`96` 舒适净空带、连续舰体扫掠和最终 Domain 校验；仅消除逐段状态深复制、恒定环境重复采样、区域类型重复解析、逐段诊断写入和样本数组重复构造。
- 长列岛 11v11 的候选 `2922`、字段查询 `175320`、已接受模拟段 `148972`、地形拒绝候选 `471` 与优化前一致，路线失败/岸线碰撞/航迹失败/结束等待仍为 `0`。
- 长列岛导航 mean/P95/P99 由 `14.245/37.686/45.818ms` 改善至 `11.127/32.262/40.813ms`，Tick 由 `31.392/61.334/70.590ms` 改善至 `28.382/55.695/65.787ms`；字段 P95/P99 保持 `1.176/1.600ms`。
- 港湾与环形泻湖 Tick P95/P99 分别改善至 `37.923/45.966ms`、`27.364/39.595ms`，四项失败仍为 `0`；开阔 11v11 未加载字段且导航也由通用运动积分减分配受益。
- 专项回归更新为 `navigation_collision_field_test.gd 70/70`，其余航迹 `28/28`、路线恢复 `10/10`、海岸运行时 `12306/12306`、地形作者 `10/10` 通过。绝对性能、带画面与人工验收仍未满足关闭条件。

### 19.3 2026-08-05 限制水域与精确窄相优化

- 碰撞场 Schema/算法升级至 v2，在同一次 `8 × 8` 格遍历中累计浅水/礁区限制位；字段只证明确定不相交，不确定位仍回退权威区域。20 图确定性重烘焙、硬地形 `19737/19737` 零误判和 `33479` 次限制水域精确查询避免门禁通过。
- 修复 DDA 在终点恰落栅格边界时可能因浮点先后顺序越过目标格的问题；Python 烘焙器与 GDScript 查询器使用同一“已到达轴不得继续前进”口径。
- 精确岸线回退新增 `256` 单位边索引，短段只扫扩张包围盒附近的权威多边形边；港湾局部运动区预排序/预计算，并只在保守可达包络可能跨越边界时逐样本重采样。
- 最终 11v11 导航 mean/P95/P99：港湾 `6.265/10.530/11.048ms`、长列岛 `4.432/7.721/9.243ms`、环形泻湖 `5.050/7.985/10.425ms`；Tick mean/P95/P99 分别为 `22.075/30.814/36.582ms`、`16.097/22.415/30.261ms`、`16.589/22.479/26.848ms`，四项失败均为 `0`。
- 长列岛候选、字段查询、已接受模拟段和地形拒绝候选仍为 `2922/175320/148972/471`。第 3 候选及以后仍有 `39.9%` 实际胜出，三图严格预测后缀可复用率仅 `2.8%-4.3%`，因此未固定砍候选或缓存旧控制。
- 专项回归更新为 `navigation_collision_field_test.gd 74/74`，航迹 `28/28`、路线恢复 `10/10`、海岸运行时 `12306/12306`、地形作者 `10/10` 通过。绝对 `4/8ms`、字段 P99 `2ms`、带画面与人工验收仍未满足关闭条件。
- 共享实现继续以同一 11v11 舰队推广复验开阔与全部十张岸岛；十张岸岛各 `400 Tick` 的四项故障仍为 `0`，导航 mean/P95/P99 平均 `4.598/7.590/9.124ms`，最轻双岛长水道 `3.513/5.864/7.049ms`、最重港湾 `6.288/10.446/10.984ms`。开阔海域第 `327 Tick` 正常结束，导航 `4.209/8.111/10.101ms`；完整逐图表见 `t00` 14.13。

### 19.4 2026-08-05 长列岛 Tick 阶段审计

- 性能模式已按权威 Tick 顺序记录互斥阶段，长列岛 11v11×400 Tick 的互斥均值合计 `16.038ms`，对总 Tick `16.039ms` 的未归类残差仅 `0.010ms`。
- 单位侦查观测、局部航迹、权威移动、AI 决策均值占比分别为 `30.3%/27.5%/14.0%/8.2%`；非导航合计 `67.0%`，战略+局部导航合计仍为最大系统家族 `33.0%`。
- 战略路线只完成 `22` 条，但单条 A* mean/P95/max `13.013/17.058/17.204ms`，使战略阶段 P99 达 `19.504ms`；单位侦查 mean/P95/P99 为 `4.846/5.127/5.261ms`，是稳定常驻热点。完整表与下钻见 `t00` 14.14。
- 相关回归为碰撞航迹 `74/74`、AI 观察 `14/14`、AI 量化 `58/58`，四项运行故障为 `0`。本项完成统计与优先级重排，不表示侦查优化、A* 可抢占或整体性能已经关闭。
