...
| 基础单元 | `4 × 4` 世界单位 | 与现有小尺度地形查询精度相容，最终值需用差分测试确认 |
| 栅格尺寸 | `1536 × 864` | 当前地图下的基础尺寸 |
| 静态占用 | `1 bit/cell` | 岛屿、硬岸或地图外 |
| 距离下界 | `uint16/cell` | 单位为世界单位或固定量化单位 |

估算内存：

- 单层 1-bit 占用约 `162 KiB`。
- 完整 OR-Mipmap 约为基础位图的 `4/3`，合计约 `216 KiB`。
- 单层 `uint16` 距离场约 `2.53 MiB`。
- 只加载当前地图；不得一次常驻十图全部字段。

若 `4` 世界单位不足以满足尖角、凹岸和大型舰切线精度，应优先保持精确窄相并对近岸单元标记“不确定”，而不是直接把全图提升到美术像素分辨率。
...
- `collision_field_usec` 的单 Tick P95 目标 `<= 1ms`、P99 目标 `<= 2ms`；若参考机器首轮基线表明目标不合理，只能用保留正确性的实测证据修订。
...
- [ ] 相同输入连续烘焙两次，所有 `.tscf` 与 manifest 字节一致。
- [ ] 修改任一障碍点、地图尺寸或 navigation revision 后，旧字段必定被拒绝。
- [ ] 视觉 PNG、参考 Mask 或小地图变化不会单独改变规则碰撞字段。
- [ ] 十张海岸地图的字段尺寸、cell size、世界变换和 checksum 与对应地形定义一致。
- [ ] 损坏头、截断 payload、非法 offset、checksum 不符和未知版本均稳定报错并回退。

### 15.2 宽相/窄相差分测试

- [ ] 对十图执行固定种子的随机点、随机带半径线段和切线查询，与多边形权威结果比较。
- [ ] `DefinitelyClear` 对权威查询的假阴性必须为 `0`。
- [ ] 记录保守假阳性率；高假阳性只能增加窄相，不得通过放松距离下界消除。
- [ ] 覆盖凸岸、凹角、尖角、薄障碍、岛间窄口、地图四边与多障碍相邻区域。
- [ ] 覆盖当前三档导航半径 `20 / 32 / 46` 及边界上下浮动值。

### 15.3 动力学一致性测试

- [ ] 同一初始状态和控制序列由规划展开与逐 Tick Domain 执行后，每 Tick 位置、航向和速度在统一容差内一致。
- [ ] 覆盖静止起步、全速、制动、左右满舵、部分舵、先制动后倒车、水流和移动倍率变化。
- [ ] 禁止重新引入 `1.5s` 粗步长作为安全结论；调试预览可以降采样显示，但不得降采样判定。

### 15.4 近岸航迹专项

- [ ] 平行岸线全速航行，净空不随重规划逐步收缩到碰撞边界。
- [ ] 舰首背向/侧向目标时的全速转弯不会以舰体外缘擦岸。
- [ ] 目标直线可达但初始转弯弧线不安全时，能够选择直行后转、减速转或外扩转向。
- [ ] 进入窄口前能够提前减速；通过后恢复合理推进。
- [ ] 靠近凸角与凹角时不发生粗采样跨越或中心线安全、舰体擦岸。
- [ ] 当前目标、玩家连续航点、AI 战术目标和接替增援舰使用同一安全验证。

### 15.5 碰撞恢复专项

- [ ] 人工构造不可避免触岸后，当前危险控制在下一 Tick 前失效。
- [ ] 恢复能够利用碰撞法线选择制动/脱离/倒车，不永久原地重试同一控制。
- [ ] 恢复后保留原语义目标；只有走廊失效时才重建战略路线。
- [ ] `NavigationCollisionContractViolated` 在自动测试中为零；故意注入错误字段时能够命中。

### 15.6 回归与性能

- [ ] `trajectory_navigation_test.gd`、`ai_route_recovery_test.gd`、`coastal_runtime_test.gd` 全部通过。
- [ ] 十张近岸地图各运行 `1` 个固定种子烟测；出现新异常时最多扩展到 `3` 个种子定位。
- [ ] 港湾、长列岛、环形泻湖执行 11v11 `400 Tick` 定向复验，记录岸线碰撞、路线失败、等待、P95/P99 与碰撞场命中率。
- [ ] 带画面和 Headless 的固定输入轨迹事实一致。
- [ ] 既有开阔海域不因加载空碰撞字段产生额外显著成本。

## 16. 实施阶段

### 阶段 A：建立失败夹具与量测

- [ ] 固化至少三个当前可复现的“规划安全但 Domain 擦岸”场景，保存地图、单位、位置、航向、速度、目标、控制与碰撞障碍。
- [ ] 增加规划样本与实际样本差异、首个分歧 Tick 和静态碰撞事件统计。
- [ ] 在不改变行为的前提下记录当前候选地形查询耗时和精确多边形调用数。

完成门禁：失败夹具在旧实现稳定失败，且能区分“路线拓扑不可达”“局部弧线不安全”和“预测/执行不一致”。

### 阶段 B：先修复正确性，不依赖碰撞场

- [ ] 抽出同固定 Tick 的公共控制序列展开。
- [ ] 局部候选每个短段先直接使用现有带半径精确连续扫掠。
- [ ] 最终候选完整验证后才提交。
- [ ] Domain 碰撞立即使当前计划和控制失效。

完成门禁：阶段 A 的擦岸夹具全部通过，`NavigationCollisionContractViolated=0`。即使此阶段性能暂未达标，也不得退回旧中心线/终点近似。

### 阶段 C：烘焙碰撞字段

- [ ] 实现 BitMask、距离下界、manifest、checksum、加载与过期回退。
- [ ] 接入十图生产流水线和重复构建确定性验证。
- [ ] 完成随机与边界差分测试，证明 `DefinitelyClear` 假阴性为零。

完成门禁：字段可安全代替远岸多边形查询，缺失或损坏字段会明确回退。

### 阶段 D：宽相/窄相集成与评分

- [ ] 所有候选使用字段宽相，近岸片段使用多边形窄相。
- [ ] 候选记录最小净空并接入净空/近岸速度评分。
- [ ] 局部规划读取下一至两个走廊门，支持进弯减速和外扩。
- [ ] 胜出候选失败时继续尝试下一候选，不直接进入 SafetyHold。

完成门禁：近岸行为专项通过，远岸精确多边形调用下降达到目标，且不增加静态岸线碰撞。

### 阶段 E：恢复、诊断与大编队复验

- [ ] 完成碰撞法线驱动的局部脱离、受限倒车和原意图恢复。
- [ ] 完成港湾、长列岛、环形泻湖 11v11 定向复验。
- [ ] 核对 `t00` 绝对性能门禁；本工单通过不自动代表港湾整体 P95/P99 已关闭。

### 阶段 F：文档与状态同步

- [ ] 按实际实现更新 `docs/35_scene_combat_domain_design.md` 的共享碰撞场加速边界。
- [ ] 更新 `docs/technical/t00_coastal_ai_performance_solution.md` 的查询预算与指标。
- [ ] 更新 `docs/technical/t01_inertial_navigation_and_emergency_avoidance.md` 的同 Tick 轨迹、净空评分和恢复流程。
- [ ] 若新增数据引用字段，更新 `docs/22_scene_environment_data_schema.md`、加载校验与负例。
- [ ] 更新 `docs/34_implementation_map.md`、`docs/00_project_status.md` 和 `AGENTS.md`；只按已有自动/人工证据标记完成度。
...
| 项目 | 优先级 | 状态 |
|---|---|---|
| 失败夹具与预测/执行差异量测 | P0 | 待实施 |
| 同固定 Tick 轨迹展开 | P0 | 待实施 |
| 带舰体半径连续候选验证 | P0 | 待实施 |
| Domain 碰撞后立即计划失效 | P0 | 待实施 |
| BitMask 与距离下界场烘焙 | P1 | 待实施 |
| 宽相/窄相共享查询 | P1 | 待实施 |
| 净空与近岸速度评分 | P1 | 待实施 |
| 定向胶囊/椭圆统一语义 | P2 | 待决策 |
| 十图与 11v11 验收 | P1 | 待实施 |
